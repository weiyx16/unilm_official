...
from utils import is_zip_path, ZipReader
...
class CachedImageFolder(DatasetFolder):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None and not isinstance(self.transform, list):
            img = self.transform(image)
        elif self.transform is not None and isinstance(self.transform, list):
            img = []
            for i in range(len(self.transform)):
                _img = self.transform[i](image)
                if isinstance(_img, list) or isinstance(_img, tuple):
                    img.extend(_img)
                else:
                    img.append(_img)
        else:
