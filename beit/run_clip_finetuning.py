...
from optim_factory import create_optimizer, get_parameter_groups, CLIPLayerDecayValueAssigner
...
def get_args():
    ...
    parser.add_argument('--data_set', default='IMNET', choices=['CIFAR', 'IMNET', 'image_folder', 'IMNET_ZIP'],
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        if 'positional_embedding' in checkpoint_model:
            ...
            num_patches = int(model.input_resolution // model.patch_size) ** 2 # model.patch_embed.num_patches
            num_extra_tokens = model.positional_embedding.shape[-2] - num_patches
    ...
    if args.layer_decay < 1.0:
        assigner = CLIPLayerDecayValueAssigner(list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)))
    else:
        assigner = None
...
if __name__ == '__main__':
    ...
    opts.output_dir = os.path.join(opts.output_dir, opts.tag)
    if opts.output_dir:
